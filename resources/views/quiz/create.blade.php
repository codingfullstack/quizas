    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 border-2 rounded-xl p-2 bg-black">
            <div class=" overflow-hidden shadow-xl sm:rounded-lg">
                @include('formError')
                <h2 class='text-3xl  ml-10'>Create new Quiz</h2>
                <Form method="POST" class="text-black" action="{{ route('quiz.store') }}">
                    @csrf

                    <input
                        class="ml-10 my-2 h-8 w-10/12 rounded-xl border-lime-500 border-2 focus:border-lime-500  focus:ring-0"
                        type="text" name="name" id="quiz-name" value="{{ old('name') }}" placeholder="Quiz name"
                        required>
                    <textarea class="ml-10 my-2  w-10/12 rounded-xl border-lime-500 border-2 focus:border-lime-500  focus:ring-0"
                        name="description" id="" cols="30" rows="5" placeholder="Description..." required
