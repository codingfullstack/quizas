    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 border-2 rounded-xl p-2 bg-black">
            <div class=" overflow-hidden shadow-xl sm:rounded-lg">
                @include('formError')
                <h2 class='text-3xl text-white  ml-10'>Edit blog</h2>
                <Form method="POST" class="text-black" action="{{ route('blog.update', $blog->id) }}">
                    @csrf
                    @method('PUT')
                    <input
                        class="ml-10 my-2 h-8 w-10/12 rounded-xl border-lime-500 border-2 focus:border-lime-500  focus:ring-0"
                        type="text" name="title" id="blog-name" value={{$blog->title}} placeholder="Blog Title"
                        required>
                    ...
                    <div class="ml-10 my-2">
                        @foreach ($categories as $category)
                            ...
                            <input class="mx-2 my-2" type="radio" name="category" id={{ $category->id }} value={{ $category->id }} {{ $category->id == $blog->categories[0]['pivot']['category_id'] ? 'checked' : '' }} />
                        @endforeach
                    </div>
