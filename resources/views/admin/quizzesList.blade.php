...
@section('content')
    <main class="bg-gray-900 ml-2 w-screen">
        <div class="bg-white w-1/2 mx-auto my-5 rounded-xl">
            <h2 class="border-b-2 uppercase text-xl p-2 font-semibold">Quizzes list</h2>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    ...
                    <tbody>
                        @foreach ($Quizzes as $Quiz)
                            <tr class="bg-white border-b  ">
                                ...
                                <td class="px-6 py-4">
                                    @can('delete', $Quiz)
                                        <form class="inline" action="{{ route('quiz.destroy', $Quiz->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                class="text-white bg-red-500 hover:bg-red-600 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2"
                                                type="submit">Delete</button>
                                        </form>
                                    @endcan
                                    @if ($Quiz->suspended)
                                        <button type="button"
                                            class="text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2"
                                            data-blog-id="{{ $Quiz->id }}">
                                            Suspended
                                        </button>
                                    @else
                                        <button type="button"
                                            ...
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                ...
                                                <div class="px-6 py-6 lg:px-8">
                                                    ...
                                                    <form method="POST" id="unpublishForm" class="space-y-6"
                                                        action="{{ route('adminBlog', ['category' => 'quizzes', 'id' => '__ID__']) }}">
                                                        @csrf
                                                        @method('POST')
                                                        <div>
                                                            <label for="message"
                                                        ...
                                                        <div class="flex">
                                                            <div class="flex items-start">
                                                                ...
                                                                <label for="check"
                                                                    class="ml-2 text-sm font-medium text-gray-900">Are you
                                                                    sure
                                                                </label>
                                                            </div>
                                                        </div>
                ...
                <div class="p-5">{{ $Quizzes->links() }}</div>
            </div>
        </div>
    </main>
    <script>
